"""
RSR demo: IEEE 57-bus DC-OPF blackout reliability.

Estimates the probability of a system-wide blackout for the IEEE 57-bus power
...
The model is the multi-state DC-OPF blackout case from Chan et al. (2024):
  - 137 failable components: 7 generator buses (4 states), 50 ordinary buses
    (2 states) and 80 branches (2 states);
...
Data is read from the `ieee57` network dataset (edges/probs + MATPOWER case)
...
DEFAULT_DATASET = HERE.joinpath("../../../network-datasets/datasets/ieee57/v1")
...
def load_dataset(dataset: Path):
    """Load the ieee57 probs and return the DC-OPF system function factory."""
    ...
    return probs_dict, make_dcopt_sfun, str(data_dir / "ieee57.m")
...
def main(
    ...
    dataset: Path = typer.Option(DEFAULT_DATASET, help="ieee57 dataset version dir (data/ + scripts/)"),
    threshold: float = typer.Option(54.1, help="Blackout size (%) above which the system fails"),
    ...
    """Estimate P(blackout) and the critical failure modes for the IEEE 57-bus grid."""
    dev, multi_devices, device_list = resolve_devices(device, devices)
    n_workers = resolve_workers(n_workers)

    print("=" * 64)
    print("RSR demo — IEEE 57-bus DC-OPF blackout reliability")
    print("=" * 64)
